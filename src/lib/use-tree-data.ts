// This is a modified version of react-stately/data
// Direct patching via `pnpm patch` is not possible since it would require editing compiled React code

// ESLint and TypeScript checking are disabled as this project uses stricter settings
 ...
 */
export function useTreeData<T extends object>(
  ...
  let [tree, setItems] = useState<TreeDataState<T>>(() =>
    buildTree(initialItems, new Map()),
  );
  const [prevItems, setPrevItems] = useState(initialItems);
  if (JSON.stringify(prevItems) !== JSON.stringify(initialItems)) {
    setPrevItems(initialItems);
    setItems(buildTree(initialItems, new Map()));
  }
  let { items, nodeMap } = tree;

  let [selectedKeys, setSelectedKeys] = useState(
    new Set<Key>(initialSelectedKeys || []),
  );
  ...
  return {
    ...
    move(key: Key, toParentKey: Key | null, index: number) {
      setItems(({ items, nodeMap: originalMap }) => {
        ...
        let { items: newItems, nodeMap: newMap } = updateTree(
          items,
          key,
          () => null,
          originalMap,
...
function moveItems<T extends object>(
  ...
  function postorder(child) {
    ...
    if (
      ...
      (toParent?.children
        ? toParent.children.indexOf(child)
        : items.indexOf(child)) < originalToIndex
    ) {
      toIndex--;
    }
  }
